...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `requestcompletionstat` AS select year(`c`.`submitteddate`) AS `year`,count(`c`.`claimid`) AS `allrequests`,avg((case when (`c`.`status` <= 7) then 1 else NULL end)) AS `avgcompleted`,((count((case when (`c`.`status` >= 7) then 1 else NULL end)) / count(`c`.`claimid`)) * 100) AS `pcntcompleted`,avg((case when (`c`.`status` >= 7) then (to_days(`ca`.`auditwhen`) - to_days(`c`.`submitteddate`)) else NULL end)) AS `avgduration` from (`acebody`.`claimdetails` `c` left join `acebody`.`claimassigndetails` `ca` on((`c`.`claimid` = `ca`.`claimid`))) group by year(`c`.`submitteddate`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-04-28  1:31:39
